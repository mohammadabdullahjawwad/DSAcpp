...
public:
    ...
    T& operator[](string key) {
        ...
        if(index == NULL) {
            ...
            T garbage; // We insert garbage value
            insert(key, garbage);
            index = search(key);
        }
        return *index; // We return the bucket by reference
    }
...
int main() {
    ...
    if(price == NULL) {
        ...
        cout << "Price of " << query << " " << *price << endl; // Prints the value stored in the address returned by search() function
    }
    ...
    menu["Dosa"] += 10;
    // Search
    cout << "Price of " << "Dosa" << " " << menu["Dosa"] << endl;
